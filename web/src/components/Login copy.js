...
export default class Login extends Component {
	...
	handleSubmit(e) {
		const { email, password, emailError, emailError2, passwordError, formSubmitted } = this.state;
    ...
    render() {
      ...
      const { email, password, passwordError, emailError, emailError2, formSubmitted, authError } = this.state;
			...
			<div className="card shadow-sm border-0 px-3 rounded-2 mb-3 py-4 mx-auto mt-5">
				...
				<div className="card-body">
					{!authError && formSubmitted
						? <div className="alert alert-success mt-2">You are logged in successfully.</div>
						: ''
					}
					{ authError && formSubmitted
						? <div className="alert alert-danger mt-2">Please use valid credentails to login.</div>
						: ''
					}
					<form action="/" onSubmit={(e) => this.handleSubmit(e)} encType="multipart/form-data" autoComplete="off">
						<div className="form-group">
							...
							<input name="email" type="email" className="form-control" placeholder="Email" value={email} onChange={this.handleChange} onBlur={this.handleEmailBlur} />
								{ emailError
									? <div className="alert alert-danger mt-2">Se requiere el email.</div>
									: ''
								}
								{ emailError2
									? <div className="alert alert-danger mt-2">Email invalido.</div>
									: ''
								}
						</div>
						<div className="form-group">
							...
							{ passwordError
								? <div className="alert alert-danger mt-2">La contraseña es requerida.</div>
								: ''
							}
						</div>
